...
3. In PowerShell, set the values from your existing Koyeb configuration. Do not commit them or paste them into chat.
...
& .\.venv\Scripts\python.exe -m pip install -r requirements-mtproto-recovery.txt
```
...
Use the campaign ID from the bot's campaign report/export.

```powershell
# 1. Read-only: lists the exact first ten channel/message pairs.
& .\.venv\Scripts\python.exe scripts\cleanup_campaign_mtproto.py --campaign cmp_YOUR_ID --limit 10

# 2. Delete only that ten-channel pilot and inspect those channels in Telegram.
& .\.venv\Scripts\python.exe scripts\cleanup_campaign_mtproto.py --campaign cmp_YOUR_ID --limit 10 --confirm

# 3. If the pilot succeeds, process every remaining tracked post.
& .\.venv\Scripts\python.exe scripts\cleanup_campaign_mtproto.py --campaign cmp_YOUR_ID --confirm
```
